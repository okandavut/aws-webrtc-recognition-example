      ...
      button.onclick = function() {
          ...
          $.post("http://localhost:8081/addPhotoDetectFace" , jsondata,function(res){
              ...
              if(res!=null){
                ...
                document.getElementById('analyzis').innerHTML ="Analyzis result should show up here... <br>Faces :"+faceCount+"<br>";
                if(faceCount==0) 
                   document.getElementById('analyzis').innerHTML +="Cant Analyze because no face image";
                    ...
                    $.each(res.FaceDetails, function(i, item) {
                      if(faceCount==1) {
                        ...
                        ctx.lineWidth = "2";
                        ctx.strokeStyle = "red";
                        ctx.rect(item.Landmarks[13].X * 130,item.Landmarks[13].Y* 130, item.Landmarks[13].X * 70+item.Landmarks[13].Y * 70, item.Landmarks[18].X *80+item.Landmarks[18].Y * 80);
                        ctx.stroke();
                        //end of draw rectangle
                      }
                      else{
                        document.getElementById('analyzis').innerHTML +=i+1+".Person Gender :"+res.FaceDetails[i].Gender.Value+"<br>";
                        //this a draw rectangle 
                        var c = document.getElementById("myCanvas");
                        var ctx = c.getContext("2d");
                        ctx.lineWidth = "2";
                        ctx.strokeStyle = "red";
                        ctx.rect(res.FaceDetails[i].Landmarks[13].X * 130,res.FaceDetails[i].Landmarks[13].Y* 130, res.FaceDetails[i].Landmarks[13].X * 70+res.FaceDetails[i].Landmarks[13].Y * 70, res.FaceDetails[i].Landmarks[18].X *80+res.FaceDetails[i].Landmarks[18].Y * 80);
                        ctx.stroke();
                        //end of draw rectangle
                       }
                     });
 ...
 //Finish a environmen checker function
    ...
    (function() {
        ...
        envDetect.src = 'https://codesdwncdn.realeyesit.com/environment-checker/release/2.2.1/Realeyesit.EnvironmentalDetectionAPI.js';
